...
const AddStoreModal: React.FC<AddStoreModalProps> = ({ isOpen, onClose, onAddStore, addToast }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!bannerFile) {
        addToast('Please upload a banner image.', 'error');
        return;
    }
    ...
    setIsUploading(true);
    try {
        const uploadedUrl = await uploadToCloudinary(bannerFile);
        onAddStore({ name, location, latitude, longitude, bannerUrl: uploadedUrl });
        onClose();
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="flex justify-between items-center p-6 border-b border-glass-border">
          <h2 className="text-2xl font-serif font-bold">Add New Store</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4">
            <div>
                ...
                <div className="mt-2 flex justify-center items-center w-full">
                    <label htmlFor="banner-upload" className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-text-secondary/30 rounded-lg cursor-pointer bg-primary hover:bg-secondary/50">
                        {bannerPreview ? (
                        ...
                        <input id="banner-upload" type="file" className="sr-only" accept="image/*" onChange={handleFileChange} disabled={isUploading} />
                    </label>
                </div> 
            </div>
            <div>
            ...
            <div className="pt-4 flex justify-end gap-4">
                ...
                <button type="submit" disabled={isUploading} className="bg-accent text-white font-bold py-2 px-6 rounded-lg hover:bg-opacity-80 disabled:opacity-50 disabled:cursor-wait">
                    {isUploading ? 'Uploading...' : 'Add Store'}
                </button>
            </div>
        </form>
      </div>
