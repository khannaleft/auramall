...
export const initiatePayUPayment = async (details: PaymentDetails): Promise<void> => {
    ...
    // PayU's 'productinfo' field has a 100-character limit.
    // We must truncate it here to ensure the hash we generate matches the one
    // PayU will use in its response webhook, which prevents a hash mismatch.
    let productinfo = cartItems.map(item => item.name).join(', ');
